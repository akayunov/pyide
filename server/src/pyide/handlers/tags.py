...
class Tags(web.View):
    async def get(self):
        ...
        if file_path in AST_PARSER:
            global_scope = [
                i for i in AST_PARSER[file_path]['ast_tree'].child_parent_scope_id_links
                if AST_PARSER[file_path]['ast_tree'].child_parent_scope_id_links[i]['parent_scope_id'] is None
            ][0]

            for tag_name in AST_PARSER[file_path]['ast_tree'].scope_id_structure.get(global_scope, {}):
                # TODO add test on [0] part
                if AST_PARSER[file_path]['ast_tree'].scope_id_structure[global_scope][tag_name][0].child_scope_id:
